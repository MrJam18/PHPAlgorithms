...
class User
{
    ...
    {
        try {
            return $this->identityMap->get(static::CLASSNAME, $id);
        }
    ...
    {
        $saved = $this->identityMap->getAllByClassName(static::CLASSNAME);

        $user = null;
        foreach ($saved as $savedUser) {
            if($savedUser->getLogin() === $login) return $user = $savedUser;
        }
